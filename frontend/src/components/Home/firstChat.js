...
function FirstChatSection() {
  ...
  const [userInputs, setUserInputs] = useState(["first", "second"]); // Example user inputs
      ...
      <AnimatePresence>
        {userInputs.map((input, index) => (
          <motion.div
            ...
            exit={{ opacity: 0, y: 10 }}
            key={index}
            className="w-auto bg-slate-600 h-6 rounded-tr-xl rounded-br-sm absolute left-0 bottom-32 border-[1px] myborder px-2 flex gap-10 items-center"
